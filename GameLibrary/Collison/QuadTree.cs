...
namespace GameLibrary.Collison
{
    public class QuadTree<T> where T : GameLibrary.Object.Object
    {
        ...
        public void Insert(T quadObject)
        {
            lock (syncLock)
            {
                ...
                Rectangle bounds = quadObject.Bounds.Rectangle;//new Rectangle((int)quadObject.Position.X, (int)quadObject.Position.Y, (int)quadObject.Size.X, (int)quadObject.Size.Y); //???
                if (root == null)
                {
        ...
        private void Query(Rectangle bounds, QuadNode node, List<T> results)
        {
            lock (syncLock)
            {
                ...
                if (bounds.Intersects(node.Bounds))
                {
                    ...
                    {
                        if (bounds.Intersects(quadObject.Bounds.Rectangle))
        ...
        private void InsertNodeObject(QuadNode node, T quadObject)
        {
            lock (syncLock)
            {
                if (!node.Bounds.Contains(quadObject.Bounds.Rectangle))
                ...
                if (!node.HasChildNodes() && node.Objects.Count + 1 > maxObjectsPerLeaf)
                {
                    ...
                    foreach (T childObject in childObjects)
                    {
                        foreach (QuadNode childNode in node.Nodes)
                        {
                            ...
                            if (childNode.Bounds.Contains(childObject.Bounds.Rectangle))
                            {
                ...
                foreach (QuadNode childNode in node.Nodes)
                {
                    if (childNode != null)
                    {
                        if (childNode.Bounds.Contains(quadObject.Bounds.Rectangle))
                        {
